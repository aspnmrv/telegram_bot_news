...
async def get_accept(event):
    ...
    if await is_expected_steps(user_id, [11]):
        ...
        keyboard = await get_keyboard(["Получить новости 💜"])
        text = "Ура, почти все готово!😇\n\nЧтобы запустить фильтрацию новостей, жмякай на Получить новости 💜\n\n" \
...
@bot.on(events.NewMessage(pattern="Получить новости 💜"))
async def get_first_news(event):
    """"""
    user_id = event.message.peer_id.user_id
    current_step = await _get_current_user_step(user_id)

    if await is_expected_steps(user_id, [12]):
        await _update_current_user_step(user_id, 367)
        text = "Сейчас я сформирую краткие выдержки последних новостей из выбранных каналов ☺️\n\n" \
               "В дальнейшем для получения новостей можешь выбрать в меню команды:\n" \
               "/summary - получение кратких выдержек новостей\n" \
               "/news - получение новостей в виде репостов\n"
        await event.client.send_message(event.chat_id, text, buttons=Button.clear())
        await update_data_events_db(user_id, "get_first_news", {"step": current_step})

        await get_summary(event)
    else:
        pass
